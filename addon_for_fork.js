...
// NEW: Read environment variable for GDrivePlayer
const ENABLE_GDRIVEPLAYER_PROVIDER = process.env.ENABLE_GDRIVEPLAYER_PROVIDER !== 'false'; // Defaults to true if not set or not 'false'
console.log(`[addon.js] GDrivePlayer provider fetching enabled: ${ENABLE_GDRIVEPLAYER_PROVIDER}`);
...
const { getGDrivePlayerStreams } = require('./providers/gdriveplayer.js'); // NEW: Import from gdriveplayer.js
...
builder.defineStreamHandler(async (args) => {
    ...
    const providerFetchFunctions = {
        ...
        // GDrivePlayer provider with cache integration
        gdriveplayer: async () => {
            if (!ENABLE_GDRIVEPLAYER_PROVIDER) { // Check if GDrivePlayer is disabled
                console.log('[GDrivePlayer] Skipping fetch: Disabled by environment variable.');
                return [];
            }
            if (!shouldFetch('gdriveplayer')) {
                console.log('[GDrivePlayer] Skipping fetch: Not selected by user.');
                return [];
            }
            
            // Try to get cached streams first
            const cachedStreams = await getStreamFromCache('gdriveplayer', tmdbTypeFromId, tmdbId, seasonNum, episodeNum);
            if (cachedStreams) {
                console.log(`[GDrivePlayer] Using ${cachedStreams.length} streams from cache.`);
                return cachedStreams.map(stream => ({ ...stream, provider: 'GDrivePlayer' }));
            }
            
            // No cache or expired, fetch fresh
            try {
                console.log(`[GDrivePlayer] Fetching new streams...`);
                const streams = await getGDrivePlayerStreams(tmdbId, tmdbTypeFromId, seasonNum, episodeNum);
                
                if (streams && streams.length > 0) {
                    console.log(`[GDrivePlayer] Successfully fetched ${streams.length} streams.`);
                    // Save to cache
                    await saveStreamToCache('gdriveplayer', tmdbTypeFromId, tmdbId, streams, 'ok', seasonNum, episodeNum);
                    return streams.map(stream => ({ ...stream, provider: 'GDrivePlayer' }));
                } else {
                    console.log(`[GDrivePlayer] No streams returned.`);
                    // Save empty result
                    await saveStreamToCache('gdriveplayer', tmdbTypeFromId, tmdbId, [], 'failed', seasonNum, episodeNum);
                    return [];
                }
            } catch (err) {
                console.error(`[GDrivePlayer] Error fetching streams:`, err.message);
                // Save error status to cache
                await saveStreamToCache('gdriveplayer', tmdbTypeFromId, tmdbId, [], 'failed', seasonNum, episodeNum);
                return [];
            }
    ...
    try {
        ...
        const providerResults = await Promise.all([
            ...
            timeProvider('GDrivePlayer', providerFetchFunctions.gdriveplayer()) // NEW: Add GDrivePlayer provider
        ]);
        ...
        const streamsByProvider = {
            ...
            'GDrivePlayer': ENABLE_GDRIVEPLAYER_PROVIDER && shouldFetch('gdriveplayer') ? filterStreamsByQuality(providerResults[9], minQualitiesPreferences.gdriveplayer, 'GDrivePlayer') : [] // NEW: Add GDrivePlayer provider
        };
        ...
        combinedRawStreams = [];
        const providerOrder = ['ShowBox', 'Hianime', 'Xprime.tv', 'HollyMovieHD', 'Soaper TV', 'VidZee', 'MP4Hydra', 'GDrivePlayer', 'Cuevana', 'VidSrc']; // NEW: Add GDrivePlayer provider
